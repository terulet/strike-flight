...
describe('Ritmo inicial de Canyon Run', () => {
  it('enlaza las cinco piezas sin choque con gas y correccion aerea sencilla', () => {
    const track = buildCanyonRun();
    const race = new RaceManager(track);
    race.begin();
    while (race.state === 'COUNTDOWN') race.step(SIM_DT, neutral());
    const target = track.terrainFeatures.find((feature) => feature.kind === 'rockgarden')!.endX;
    while (race.state === 'RACING' && race.raceTime < 24 && race.bike.x < target) {
      let lean = 0;
      if (isAirborne(race.bike)) {
        const targetX = Math.min(target, race.bike.x + Math.max(2, race.bike.vx * 0.22));
