...
export class TrackBuilder {
  ...
  tabletop(approachLength: number, height: number, topLength: number, landingLength: number): this {
    return this.rampUp(approachLength, height).flat(topLength).slope(landingLength, -height);
  }
  ...
  stepUp(
    ...
    return this.rampUp(takeoffLength, takeoffHeight)
      .gapValley(gapLength, gapDepth)
      .slope(landingLength, landingRise)
      .flat(platformLength);
  }

  dropOff(approachLength: number, dropLength: number, dropHeight: number, exitLength: number): this {
    return this.flat(approachLength).slope(dropLength, -Math.abs(dropHeight)).flat(exitLength);
  }

  rockGarden(length: number, heights: number[]): this {
    if (heights.length === 0) return this.flat(length);
    const baseY = this.y;
    const stepLen = length / (heights.length + 1);
    for (const height of heights) {
      this.x += stepLen;
      this.y = baseY + Math.max(0, height);
      this.points.push({ x: this.x, y: this.y });
    }
    this.x += stepLen;
    this.y = baseY;
